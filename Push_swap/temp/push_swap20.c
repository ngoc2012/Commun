...
/*   Updated: 2023/01/26 03:30:00 by minh-ngu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	to_right(t_stack *st)
{
	...
	last = (st->len - st->push) / 3;
	if (st->len - st->push - 3 * last == 2)
		last1 = 2 * (last + 1);
	else
		last1 = 2 * last;
	last = st->len - st->push - last1;
	...
	while (st->len - st->push != last)
	{
		...
		if (st->cur[st->push].a_p < st->len - last)
		{
			...
			if (st->push > 1 && st->cur[st->push - 1].a_p < st->len - last - last1 / 2 && (st->cur[st->push - 2].a_p >= st->len - last - last1 / 2 || st->cur[st->push - 2].a_p < st->len - last - last1))
			{
