...
/*   Updated: 2023/11/26 15:55:14 by ngoc             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	main_signal_handler(int sig)
{
	...
	if (sig == SIGPIPE)
	{
		...
		std::map<int, Request*>	sk_client_request = g_host->get_sk_client_request();
		for (std::map<int, Response*>::iterator	it = sk_response.begin();
			it != sk_response.end(); ++it)
			g_host->delete_response(it->first);
		for (std::map<int, Request*>::iterator	it = sk_client_request.begin();
