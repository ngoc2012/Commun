...
/*   Updated: 2023/11/23 15:28:36 by ngoc             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void  	Host::add_sk_2_master_set(int new_sk, Server* s)
{
	if (new_sk > _max_sk)
		_max_sk = new_sk;
...
{
	add_sk_2_master_set(new_sk, s);
	_sk_client_request[new_sk] = new ClientRequest(new_sk, this);
}

void	Host::new_response_sk(int new_sk, Server* s)
{
	_sk_response[new_sk] = new Response(new_sk, this, s);
}
...
void	Host::start(void)
{
	...
	for (std::vector<Server*>::iterator it = _servers.begin(); it != _servers.end();)
	{
		...
		if (listen_sk > 0)
		{
			add_sk_2_master_set(listen_sk, *it);
	...
	do
	{
		...
		for (std::map<int, Server*>::iterator it = _sk_server.begin(); it != _sk_server.end();++it)
