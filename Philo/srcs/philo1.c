...
/*   Updated: 2023/04/09 19:19:41 by ngoc             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	now_time_interval(struct timeval *tv1, struct timeval *tv2, pthread_mutex_t *m)
{
	pthread_mutex_lock(m);
...
void	eating(t_philo *ph)
{
	...
	print(ph, &ph->last_eat, " has taken a fork\n");
	print(ph, &ph->last_eat, " has taken a fork\n");
	print(ph, &ph->last_eat, " is eating\n");
	ph->fork2 = ph->last_eat;
	...
	pthread_mutex_unlock(&ph->a->m_order);
	while (now_time_interval(&ph->last_eat, &ph->fork2, &ph->m_p) <= ph->a->t_e
...
void	sleeping(t_philo *ph)
{
	...
	pthread_mutex_unlock(&ph->a->m_order);
	while (now_time_interval(&ph->last_sleep, &ph->last_eat, &ph->m_p) < ph->a->t_s
			&& !m_get(&ph->a->died, &ph->a->m_a))
		;
	if (m_get(&ph->finished, &ph->m_p) || m_get(&ph->a->died, &ph->a->m_a))
		return ;
	//if (trylock(&ph->a->m_a) == -1)
	//	return ;
	pthread_mutex_lock(&ph->a->m_order);
...
void	check(t_academy *a)
{
	...
	while (!a->died && !finished)
	{
		...
		{
			if (now_time_interval(&tv, &a->phs[i].last_eat, &a->phs[i].m_p) > a->t_d && get_time_interval_micro(&tv, &a->phs[i].last_eat) > 0)
			{
