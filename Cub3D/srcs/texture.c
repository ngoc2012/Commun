...
/*   Updated: 2023/09/23 13:23:11 by ngoc             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_textures(t_game *g)
{
	...
	g->tex_f.img = mlx_xpm_file_to_image(g->mlx.mlx, "Stone.xpm", &g->tex_f.l, &g->tex_f.h);
	if (!g->tex_f.img)
		return (0);
	g->tex_f.addr = mlx_get_data_addr(g->tex_f.img, &g->tex_f.bpp, &g->tex_f.ll, &g->tex_f.ed);
	g->tex_c.img = mlx_xpm_file_to_image(g->mlx.mlx, "Fires.xpm", &g->tex_c.l, &g->tex_c.h);
	if (!g->tex_c.img)
		return (0);
